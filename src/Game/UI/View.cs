...
public abstract class View : UserControl
{
	...
	protected override void OnKeyDown(KeyEventArgs e)
	{
		...
		{
			case Key.Up:
				FocusDirectional(current =>
					AdjustFocusTarget(DirectionalFocusStructure?.GetUp(current), ListBoxFocusMode.LastItem));
				e.Handled = true;
				break;

			case Key.Down:
				FocusDirectional(current =>
					AdjustFocusTarget(DirectionalFocusStructure?.GetDown(current), ListBoxFocusMode.FirstItem));
				e.Handled = true;
				break;

			case Key.Left:
				FocusDirectional(current =>
					AdjustFocusTarget(DirectionalFocusStructure?.GetLeft(current), ListBoxFocusMode.LastItem));
				e.Handled = true;
				break;

			case Key.Right:
				FocusDirectional(current =>
					AdjustFocusTarget(DirectionalFocusStructure?.GetRight(current), ListBoxFocusMode.FirstItem));
				e.Handled = true;
				break;
		}
	}
