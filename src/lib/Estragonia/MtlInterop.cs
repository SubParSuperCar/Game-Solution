...
internal static partial class MtlInterop
{
	...
	[LibraryImport(SkiaLibrary)]
	[UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
	private static partial IntPtr gr_backendtexture_new_metal(
		int width,
		int height,
	...
	[LibraryImport(ObjcLibrary, EntryPoint = "sel_registerName", StringMarshalling = StringMarshalling.Utf16)]
	...
	private static IntPtr _sSelCommandBuffer;
	private static IntPtr _sSelBlitCommandEncoder;
	private static IntPtr _sSelCopyFromTexture;
	private static IntPtr _sSelEndEncoding;
	private static IntPtr _sSelCommit;
	private static IntPtr _sSelWaitUntilCompleted;

	private static void EnsureSelectorsInitialized()
	{
		if (_sSelCommandBuffer != IntPtr.Zero)
			return;

		_sSelCommandBuffer = sel_registerName("commandBuffer");
		_sSelBlitCommandEncoder = sel_registerName("blitCommandEncoder");
		_sSelCopyFromTexture =
			sel_registerName(
				"copyFromTexture:sourceSlice:sourceLevel:sourceOrigin:sourceSize:toTexture:destinationSlice:destinationLevel:destinationOrigin:");
		_sSelEndEncoding = sel_registerName("endEncoding");
		_sSelCommit = sel_registerName("commit");
		_sSelWaitUntilCompleted = sel_registerName("waitUntilCompleted");
	}
	...
	public static bool BlitTexture(IntPtr commandQueue, IntPtr sourceTexture, IntPtr destTexture, int width, int height)
	{
		...
		try
		{
			...
			var commandBuffer = objc_msgSend(commandQueue, _sSelCommandBuffer);
			...
			var blitEncoder = objc_msgSend(commandBuffer, _sSelBlitCommandEncoder);
			...
			// End encoding
			objc_msgSend_void(blitEncoder, _sSelEndEncoding);

			// Commit and wait
			objc_msgSend_void(commandBuffer, _sSelCommit);
			objc_msgSend_void(commandBuffer, _sSelWaitUntilCompleted);
